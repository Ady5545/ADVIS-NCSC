  ...
  public static async assemble(query: string, params: Record<string, any>): Promise<GeneratedAssemblyPayload> {
    ...
    try {
      ...
      for (const comp of planComponents) {
        ...
        const position: [number, number, number] = comp.position ? [comp.position[0]*scale, comp.position[1]*scale, comp.position[2]*scale] : [0, 0, 0];
        const rot = comp.rotation || [0, 0, 0];
        const size: [number, number, number] = comp.size ? [comp.size[0]*scale, comp.size[1]*scale, comp.size[2]*scale] : [0.1*scale, 0.1*scale, 0.1*scale];
        ...
        // Apply rotation
        geom.applyQuaternion(new THREE.Quaternion().setFromEuler(new THREE.Euler(rot[0], rot[1], rot[2])));
